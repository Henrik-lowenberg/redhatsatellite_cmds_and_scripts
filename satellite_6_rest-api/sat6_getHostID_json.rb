...
#  p cHostsNfoArray.class
#  p cHostsNfoArray.length
  cHosts.map { |id,name| puts "#{id} #{name}" }
#  cHosts_list.map { |id,name,last_report,subscription_status_label| puts "#{id} #{name} #{last_report} #{subscription_status_label}" }
#  p contenthosts.class
#  p contenthosts.length
#  contenthosts.each do |one,two,three,four|
#    puts one,two,three,four
=begin
  i=0
  while i < contenthosts.['results'].length
    puts contenthosts['results'][0]['name']
    i += 1
  end
=end
  # --------------------------------------------  #  
